...
## 2026-03-05 Dungeon Module Addendum

- Данжи вынесены в отдельный модуль `server.dungeon` (`DungeonService`) с минимальной интеграцией в существующие `runtime/events/structure`, чтобы изменения можно было безопасно мержить отдельно от других игровых подсистем.
- Вход в данж реализован через data-driven структуру `veyloria:dungeon_cave` (шаблон `generated:dungeon_cave` + правило спавна в `structure_spawn_rules`) с портальной точкой входа без отдельной команды.
- Для `veyloria:dungeon_cave` закреплено требование coverage: 1 вход на каждую тестовую локацию/зону (1..7), с fallback-алгоритмом генерации при неудачном случайном подборе точки.
- Инстанс данжа создаётся в отдельном измерении `veyloria:dungeon_depths`; каждый запуск получает изолированные координаты в сетке инстансов.
- Формат данжа процедурно-параметрический, но стабилен по форме: стартовая входная группа, извилистые коридоры, одна развилка, одна босс-комната и конец.
- Для прохода зафиксировано постоянное количество мобов (обычные/элитные/босс) без респавна; после полной зачистки инстанс остаётся пустым.
- Перезапуск данжа завязан на состояние группы: только после полного выхода всех участников следующий вход создаёт новый запуск.
- Масштабирование сложности сделано server-authoritative по числу вошедших игроков (до 5): HP/урон усиливаются ступенчато и некратно; если игроки входят вне боя, усиливаются только оставшиеся живые мобы.
- Базовый уровень мобов инстанса определяется по локации входа: используется верхняя граница диапазона уровня соответствующей зоны.
- Неймплейты мобов в данже унифицированы с внешним миром (уровень + полоска HP + числовые значения HP).
- Данж-AI не использует leash-эвейд; сброс цели допустим только после подтверждённой длительной недосягаемости цели.
- Вход во время активного боя в инстансе блокируется; в стартовой группе добавлен NPC-заглушка (без квестовой логики) и серверная генерация факелов/случайных сундуков.
