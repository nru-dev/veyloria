...
- враждебность `friendly/neutral/hostile` с фиксированными правилами:
  ...
  - `neutral` не атакуют игрока до провокации
  - `hostile` атакуют игрока и `friendly`
  - условные пары работают через recent-memory:
    - `friendly -> neutral` только если neutral атаковал игрока
    - `neutral -> friendly` только как self-defense
    - `hostile -> neutral` только если neutral атаковал hostile
- Common/Elite AI патчится на существующие сущности (без замены ванильных entity type):
  - `homePos` + `groupId` фиксируются на спавне
  - idle ограничен радиусом `15` блоков от home
  - combat leash `30` блоков, возврат/сброс в радиусе `3` блоков
  - если цель недостижима `100` тиков, моб уходит в evade и возвращается домой
